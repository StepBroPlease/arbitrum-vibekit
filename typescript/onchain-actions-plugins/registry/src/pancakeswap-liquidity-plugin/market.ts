...
export type PancakeSwapLiquidityMarket = {
...
const marketMap: Record<number, PancakeSwapLiquidityMarket> = {
  42161: {
    // Arbitrum One
...
export const getMarket = (chainId: number): PancakeSwapLiquidityMarket => {
  const market = marketMap[chainId];
  if (!market) {
    throw new Error(
      `PancakeSwap: no market found for chain ID ${chainId}: modify pancakeswap-liquidity-plugin/market.ts`
    );
  }
